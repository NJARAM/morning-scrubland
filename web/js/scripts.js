...
function submitfile1(baseUrl, id)
{
	...
	var xhr = new XMLHttpRequest();
	// Open the connection.
	xhr.open('POST', baseUrl+'/claim/adddocument?id='+id, true);
	// Set up a handler for when the request finishes.
	xhr.onload = function () 
	{
		if (xhr.status === 200) 
	  	{
			...
			document.getElementById('docs').innerHTML = fetch_page(baseUrl+'/claim/documents?id='+id);
	  	} else 
		{
			alert('An error occurred!');
	  	}
